<?php
	...
	function listaTodasSubCategorias($id){
		if($id == 0){
			$resultado = selectListaTodasSubCategorias();
		}
		else{
			$resultado = selectListaUmaCategoria($id);
		}
		$cat = '';
		$cont = 1;		
		while ($row = mysql_fetch_array($resultado)) {
			if($row['idCategoria'] != $cat && $cat != ''){
				echo"</li>";
				$cont++;
			}
			if($row['idCategoria'] != $cat){				
				echo"<li class='nav-header'>";
				echo"	<a data-toggle='collapse' data-target='#".$row['tituloCategoria']."'>";
				if($cont == 1)
				echo"   	<h5>".$row['tituloCategoria']." <i class='glyphicon glyphicon-chevron-down'></i></h5>";
				else
				echo"   	<h5>".$row['tituloCategoria']." <i class='glyphicon glyphicon-chevron-right'></i></h5>";
				echo"   </a>";
			}
			if($cont == 1)
			echo"   <ul class='list-unstyled collapse in' id='".$row['tituloCategoria']."'>";
			else
				echo"   <ul class='list-unstyled collapse' id='".$row['tituloCategoria']."'>";
				echo"   	<li>";
				echo"			<a href='#'> ".$row['tituloSubCategoria']." <span class='badge badge-info'>".$row['idSubCategoria']."</span></a>";
				echo"		</li>";
				echo"    </ul>";		
			$cat = $row['idCategoria'];			
		}		
